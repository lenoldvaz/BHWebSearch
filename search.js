...
function getFoodSearchResults() {
    ...
    const apiUrl = `${bhUrl}?value=${encodeURIComponent(searchTerm)}&limit=10&confidence=.7`;
    ...
    request.onload = function () {
        if (request.status >= 200 && request.status < 400) {
            ...
            data.items.forEach(food => {
                ...
                carbs.textContent = "Carbs " + food.nutrients.carbs.toFixed(2) + "g";

                const protein = card.getElementsByClassName('sr-macro protein')[0];
                protein.textContent = "Protein " + food.nutrients.protein.toFixed(2) + "g";

                const fats = card.getElementsByClassName('sr-macro fat')[0];
                fats.textContent = "Fat " + food.nutrients.fats.toFixed(2) + "g";

                const calories = card.getElementsByClassName('sr-cal-value')[0];
                calories.textContent = food.nutrients.calories.toFixed(2);
